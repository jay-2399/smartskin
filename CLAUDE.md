...
- **Plan 5 (analyse IA + résultats)** : sortie alignée sur la maquette 11 (score, profil, 16 attributs/4 sections — `src/features/analysis/attributes.ts`). **Analyse = Claude Opus 4.8 vision** (`ANTHROPIC_API_KEY`, `anthropic.ts`, adaptive thinking) par défaut, via le dispatcher `analyze.ts` basculable **Anthropic > OpenAI (gpt-5.5) > Gemini > démo**. Le « pourquoi » des produits de routine (`llm-choice.ts`) suit la même clé : **Claude Sonnet 4.6** si `ANTHROPIC_API_KEY`, sinon OpenAI. **Sans aucune clé → bilan d'exemple** (`sample.ts`). `/api/analyze` sans auth ni DB (résultat en mémoire `resultStore`). Démo testeurs : **`/resultats?demo=1`**.
