...
- **Plan 5 (analyse Gemini + résultats)** : sortie alignée sur la maquette 11 (score, profil, 16 attributs/4 sections — `src/features/analysis/attributes.ts`). Client `@google/genai` avec clé **Google AI Studio** (`GEMINI_API_KEY`) ; **sans clé → bilan d'exemple** (`sample.ts`) pour tester sans clé ni compte. `/api/analyze` sans auth ni DB (résultat en mémoire `resultStore`). Démo testeurs : **`/resultats?demo=1`**.
