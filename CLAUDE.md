...
## G. État
Design validé. Prochaine étape : plan d'implémentation (skill writing-plans) puis scaffold Next.js. **Pas encore de code.**

## H. Commandes (à compléter au scaffold)
- `dev` / `build` / `start` — _TBD à l'init Next.js_
- `prisma migrate` / `prisma studio` — _TBD_
- Déploiement : push git → Render (région Frankfurt EU)
