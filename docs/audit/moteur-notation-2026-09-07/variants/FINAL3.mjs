...
function wPos(it, barre) {
  if (it.fiche?.lowDose) return (it.pos <= 10 || it.pos < barre) ? 1.0 : 0.6;                 // efficaces < 1 % : poids plein (exception)
...
export function scoreFormule(inci, categorie, filtresUV) {
  ...
  const ctx = {
    list, barre, categorie, filtresUV: !!filtresUV, w: (it) => wPos(it, barre),
  ...
  for (const it of list) {
    ...
    const actifProuve = f.role === "active" && (f.benefitPower || 0) >= 3 && (f.risks?.irritant || 0) <= 2 && grav === (f.risks?.irritant || 0);
    if (grav >= 2 && !aMalusFixe && !dejaFactures.has(it.name)) {
      ...
      const pts = -CONFIG.malusRisque * grav * wPos(it, barre) * R.severite * (R.exposition ?? 1) * (actifProuve ? 0.5 : 1);
      score += pts;
