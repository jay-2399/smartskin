...
export const SIM = { d10: false, s5: false, d4: false, actifsW: false, dedup: false, capPerso: false, parfumUneFois: false, d8: false, d7: false, actifTop5LowDose: false, prerequisMono: false, lowDoseSousBarre: false, d8min: false, top5Gradue: false, d9: false, s5Mode: false, prereqMode: false, comedoMode: false, banMode: false, sensi3Formule: false, forceFusion: false, texture: false, uvData: false, perfumeAlias: false, ld10: false, d8Dermato: false, matchScale: false, humLowDose: false };
...
function wPos(it, barre) {
  if (it.fiche?.lowDose) return (SIM.ld10 ? (it.pos > 10 && it.pos >= barre) : (SIM.lowDoseSousBarre && it.pos >= barre)) ? 0.6 : 1.0;                 // efficaces < 1 % : poids plein (exception)
...
export function scoreFormule(inci, categorie, filtresUV) {
  ...
  const vusRisque = new Set(); let nActifsIrritants = 0;
  for (const it of list) {
    const f = it.fiche;
    if (!f) continue;
...
export function scorePerso(inci, profil, categorie, formule, filtresUV) {
  ...
  let score = F.score;
  const facts = [];
  let matchTotal = 0, capAbsolu = Infinity, strengthMax = 0; let parfumVu = false, heVu = false, sensiTotal = 0, irrTotal = 0, comedoTotal = 0;
  ...
  for (const it of list) {
    ...
    if (!SIM.d7 || w >= 0.6) strengthMax = Math.max(strengthMax, f.strength || 0);
    ...
    if (f.dryingAlcohol && (["dry"].includes(profil.skinType) || (SIM.d8Dermato && S >= 2))) {
