...
export function natureProduit(list) {
  ...
  for (const it of list) {
    ...
    if (it.fiche?.strength && (!SIM.forcePonderee || p <= 10 || it.fiche.lowDose)) forceMax = Math.max(forceMax, it.fiche.strength);
  }
...
const PREDICATS = {
  ...
  "@actifTop5": (ctx) => ctx.list.some((it) => (it.pos <= 5 || (SIM.lowDoseTop5 && it.fiche?.lowDose)) && it.fiche?.role === "active" &&
...
export function scoreFormule(inci, categorie, filtresUV) {
  ...
  for (const it of list) {
    ...
    if (grav >= 2 && !aMalusFixe && !dejaFactures.has(it.name) && actifProuve !== "gratuit") {
    ...
    if (f.fragrance && (SIM.parfumUnique ? SIM.tarifParfum : CONFIG.malusParfumFixe) > fixe) { fixe = (SIM.parfumUnique ? SIM.tarifParfum : CONFIG.malusParfumFixe); typeFixe = "parfum"; }
  ...
  return { score: clamp(score), cap, bande: bande(clamp(score)), details, couverture, metier: R.metier,
...
export function scorePerso(inci, profil, categorie, formule, filtresUV) {
  ...
  let parfumFait = false, heFait = false, sensCumul = 0, irrCumul = 0;
  ...
  for (const it of list) {
    ...
    if (!SIM.forcePonderee || w >= 0.6 || f.lowDose) strengthMax = Math.max(strengthMax, f.strength || 0);
    ...
    if (f.fragrance && (profil.sensitivity || 0) > 0 && !(SIM.persoPlafonds && parfumFait)) {
      parfumFait = true;
      const pts = -CONFIG.malusParfumSensible * profil.sensitivity * (SIM.persoPlafonds ? (CONFIG.RUBRIQUES[categorie]?.exposition ?? 1) : 1);
      score += pts;
