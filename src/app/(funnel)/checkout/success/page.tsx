...
export default async function Page({ searchParams }: { searchParams: Promise<{ session_id?: string }> }) {
  ...
  if (session_id && process.env.STRIPE_SECRET_KEY) {
    try {
      ...
      name = session.customer_details?.name ?? null; // nom du payeur → prénom du compte
    } catch {
      /* session invalide / introuvable → traité comme non payé */
    }
  }
  return <CheckoutSuccess email={email} name={name} paid={paid} sessionId={paid ? (session_id ?? null) : null} />;
}
