...
export async function POST(request: Request) {
  try {
    ...
    // Corriger la catégorie ne rend pas notable une liste qui ne l'est pas (B4).
    const reponse: Record<string, unknown> = formule.evaluable
      ? { disponible: moteurDisponible(), algoVersion: CONFIG.algoVersion,
          formule, ingredients: ficheIngredients(inci, dictionnaire(), pr) }
      : { ...scoreNonEvaluable(formule.raison), algoVersion: CONFIG.algoVersion,
          ingredients: ficheIngredients(inci, dictionnaire(), pr) };
    if (formule.evaluable && r?.etat === "ok") reponse.perso = scorePerso(inci, pr, categorie, formule, filtresUV);
    else if (formule.evaluable && r) reponse.profilManquant = r.etat;
