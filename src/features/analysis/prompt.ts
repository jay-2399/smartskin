...
export function buildPrompt(answers: Answers): string {
  return [
    ...
    "PIGMENT vs REDNESS — the single most important distinction (do not confuse them): dark_spots, post_acne_marks and tone_evenness are EXCLUSIVELY about BROWN / pigmented marks (hyperpigmentation, melasma, dark post-acne spots). Grade them above 1 ONLY if there are clearly defined DARKER (brown) marks or patches you can point to. Redness, pink/red blotchiness, flushing, irritation, visible flaking or a generally red/reactive complexion are NOT 'uneven tone' and NOT dark_spots — grade those under `redness` (and `flaking`), and keep tone_evenness / dark_spots at 1. A red, dry or irritated face must come out as redness/flaking, never as pigment. Natural freckles, a normal even complexion, an existing tan, makeup or a lighting shadow are also NOT pigment — grade 1.",
    "",
    "Blemishes:",
    ...
    "",
    "PROFILE:",
    "- skinType (inspired by Baumann): infer from per-zone shine AND the questionnaire — shine everywhere = Oily; shine on the T-zone ONLY, with clearly matte/drier cheeks = Combination; matte skin with neither shine nor tightness = Normal; flaking/tightness = Dry. CRITICAL: \"Combination\" is NOT a default label — use it ONLY when the T-zone is genuinely shinier than the cheeks; a balanced or matte T-zone = Normal, never Combination. The skinType MUST stay consistent with skinTypeBreakdown (never output \"Combination\" while describing the T-zone as \"balanced\"/matte). Blemish-prone or acne-prone is NOT the same as oily/combination — an acne-prone but non-shiny face is usually Normal or Dry, often \"sensitive\". Add \"sensitive\" for redness/reactivity (photo or declared): \"Normal, sensitive\" and \"Dry, sensitive\" are perfectly valid, common results.",
    ...
    "- behavioralLink: ONE sentence linking a REAL declared answer to a VISIBLE result, with a defensible cosmetic causality. Pick the answer MOST relevant to THIS face and VARY the angle — do NOT default to SPF (sunscreen) every time; only mention it when sun habits genuinely explain what you see. Lean on whichever truly fits: an active already tolerated (q3) → the benefit it shows, an ingredient flagged as irritating (q2) → redness/reactivity seen, a recent skin change (q5) → the visible shift, a declared priority (q1). Never invent an answer that wasn't given; if none connects honestly to what you observe, give a short neutral encouragement instead of forcing a link.",
